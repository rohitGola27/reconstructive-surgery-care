...
const galleryItems = [
  { id: 1, category: "Breast Reconstruction", pdfPath: "/documents/Nipple Sparing Mastectomy.pdf" },
  { id: 2, category: "Skin Cancer", pdfPath: "/documents/The_Buccal_Fat_Pad_Flap_for_Periorbital.24.pdf" },
  { id: 3, category: "Trauma Repair", pdfPath: "/documents/Post Traumatic Maxillary Recon.pdf" },
  { id: 4, category: "Breast Reconstruction", pdfPath: "/documents/Augmentation Mastopexy Paper.pdf" },
  { id: 5, category: "Hand Surgery", pdfPath: "/documents/Dog Bite-Article Singh.pdf" },
  { id: 6, category: "Scar Revision", pdfPath: "/documents/Fasica Lata Repair Closure.pdf" },
];
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 max-w-5xl mx-auto">
            {galleryItems.map((item, index) => (
              <button
                key={item.id}
                type="button"
                className="group relative aspect-[4/3] bg-muted rounded-xl overflow-hidden shadow-card animate-fade-up cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
                style={{ animationDelay: `${index * 100}ms` }}
                onClick={() =>
                  setSelectedPDF({
                    path: item.pdfPath,
                    title: `${item.category} - Case Study`,
                  })
                }
              >
